...
# ──────────────────────────────────────────────
class HidroelectricaDetailSensor(HidroelectricaEntity):
    """Generic sensor that promotes API fields currently stored as attributes."""
    ...
    def __init__(
        ...
        attrs_fn: Callable[[dict | None], dict[str, Any]] | None = None,
    ) -> None:
        super().__init__(coordinator, config_entry)
        self._attr_name = name
        self._attr_unique_id = f"{DOMAIN}_{key}_{self._uan}"
        self._custom_entity_id = f"sensor.{DOMAIN}_{self._uan}_{key}"
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        if not self._license_valid:
            return {"licenta": "necesara"}
        attrs = self._attrs_fn(self.coordinator.data) if self._attrs_fn else {}
        attrs["attribution"] = ATTRIBUTION
        return attrs


def _amount_from_bill(data: dict | None, field: str) -> float | None:
    bill = _get_bill_result(data)
    raw = bill.get(field)
    if raw in (None, ""):
        return None
    try:
        return round(parse_romanian_amount(str(raw)), 2)
    except (ValueError, TypeError):
        return None


def _positive_or_zero(value: float | None) -> float | None:
    if value is None:
        return None
    return round(value if value > 0 else 0.0, 2)


def _negative_credit(value: float | None) -> float | None:
    if value is None:
        return None
    return round(abs(value), 2) if value < 0 else 0.0


def _latest_usage_entry(data: dict | None) -> dict | None:
    entries = _get_usage_list(data)
    if not entries:
        return None
    return max(
        ...
        key=lambda e: (
            int(e.get("Year") or 0),
            int(e.get("Month") or 0),
        ),
    )


def _current_year_usage_entries(data: dict | None) -> list:
    entries = _get_usage_list(data)
    year = datetime.now().year
    current = [e for e in entries if int(e.get("Year") or 0) == year]
    if current:
        return current
    years = [int(e.get("Year") or 0) for e in entries if e.get("Year")]
    if not years:
        return []
    latest_year = max(years)
    return [e for e in entries if int(e.get("Year") or 0) == latest_year]


def _usage_value(entry: dict | None, field: str) -> float | None:
    if not entry:
        return None
    raw = entry.get(field)
    if raw in (None, ""):
...
def _usage_month_label(entry: dict | None) -> dict[str, Any]:
    if not entry:
        return {}
    month = int(entry.get("Month") or 0)
    return {"luna": MONTHS_NUM_RO.get(month, str(month)), "an": entry.get("Year", "")}


def _current_month_daily_average(data: dict | None) -> float | None:
    entry = _latest_usage_entry(data)
    kwh = _usage_value(entry, "value")
    if kwh is None or not entry:
        return None
    try:
        days = int(entry.get("BillingDays") or 0)
    except (ValueError, TypeError):
        return None
    if days <= 0:
        return None
    return round(kwh / days, 2)


def _current_year_usage_total(data: dict | None, field: str) -> float | None:
    entries = _current_year_usage_entries(data)
    if not entries:
        return None
    total = 0.0
    found = False
    for entry in entries:
        value = _usage_value(entry, field)
...
def _latest_billing_entry(data: dict | None) -> dict | None:
    entries = _get_billing_list(data)
    if not entries:
        return None
    return max(
        entries,
        key=lambda e: _parse_date_dmy(e.get("invoiceDate", "")) or datetime.min,
    )


def _latest_billing_amount(data: dict | None) -> float | None:
    latest = _latest_billing_entry(data)
    if not latest:
        return None
    try:
        return round(parse_romanian_amount(str(latest.get("amount", "0"))), 2)
    except (ValueError, TypeError):
        return None


def _latest_billing_field(data: dict | None, field: str) -> str | None:
    latest = _latest_billing_entry(data)
    if not latest:
        return None
...
def _payment_entries_current_year(data: dict | None, channel_filter: str) -> list:
    years = _extract_payment_years(data, channel_filter=channel_filter)
    if not years:
        return []
    year = datetime.now().year if datetime.now().year in years else max(years.keys())
    return years.get(year, [])


def _payment_total_current_year(data: dict | None, channel_filter: str) -> float:
    total = 0.0
    for entry in _payment_entries_current_year(data, channel_filter):
        try:
            total += parse_romanian_amount(str(entry.get("amount", "0")))
        except (ValueError, TypeError):
            continue
    return round(total, 2)


def _payment_count_current_year(data: dict | None, channel_filter: str) -> int:
    return len(_payment_entries_current_year(data, channel_filter))


def _first_pod_field(data: dict | None, field: str) -> str | None:
    pods = _get_pods_list(data)
    if not pods:
        return None
    value = pods[0].get(field)
    return str(value) if value not in (None, "") else None


def _contract_field(data: dict | None, field: str) -> str | None:
    prev = _get_previous_meter_read(data)
    if prev and prev.get(field):
        return str(prev[field])
    return None


def _meter_field(data: dict | None, field: str) -> str | None:
    meter = _get_multi_meter_data(data)
    if meter and meter.get(field) not in (None, ""):
        return str(meter[field])
    return None


def _window_open(data: dict | None) -> str | None:
    wd = _get_window_data(data)
    if not wd:
        return None
    return "Da" if wd.get("Is_Window_Open", "0") == "1" else "Nu"


def _window_close_date(data: dict | None) -> str | None:
    wd = _get_window_data(data)
    if not wd:
        return None
    return _compute_closing_date(wd) or None


def _latest_reading_field(
    field: str, register_filter: str | None = None
) -> Callable[[dict | None], Any]:
    def _getter(data: dict | None) -> Any:
        latest = _get_latest_meter_read(data, register_filter=register_filter)
        if not latest and register_filter == "1.8.0":
        ...
        if field == "ReadingType":
            return READING_TYPE_MAP.get(value, value)
        return value

    return _getter


def _build_additional_detail_sensors(
    coordinator: HidroelectricaCoordinator,
    config_entry: ConfigEntry,
) -> list[SensorEntity]:
    """Create promoted sensors for financial, usage and technical fields."""
    money = SensorDeviceClass.MONETARY
    measurement = SensorStateClass.MEASUREMENT
    total = SensorStateClass.TOTAL
    data = coordinator.data
    has_production = any(r.get("Registers") == "1.8.0_P" for r in _get_meter_read_list(data))

    definitions: list[tuple] = [
        ("valoare_sold", "Valoare sold", "mdi:cash", lambda d: _amount_from_bill(d, "rembalance"), "RON", money, measurement, None),
        ("valoare_credit", "Valoare credit", "mdi:cash-refund", lambda d: _negative_credit(_amount_from_bill(d, "rembalance")), "RON", money, measurement, None),
        ("suma_ultimei_facturi", "Suma ultimei facturi", "mdi:invoice-text", lambda d: _amount_from_bill(d, "billamount"), "RON", money, measurement, None),
        ("data_scadentei", "Data scadentei", "mdi:calendar-clock", lambda d: _format_duedate_yyyymmdd(_get_bill_result(d).get("duedate", "")) if _get_bill_result(d).get("duedate") else None, None, None, None, None),
        ("total_neachitat", "Total neachitat", "mdi:cash-alert", lambda d: _positive_or_zero(_amount_from_bill(d, "rembalance")), "RON", money, measurement, None),
        ("ultima_factura_suma", "Ultima factura - suma", "mdi:invoice-text-outline", _latest_billing_amount, "RON", money, measurement, None),
        ("ultima_factura_data", "Ultima factura - data", "mdi:calendar", lambda d: _latest_billing_field(d, "invoiceDate"), None, None, None, None),
        ("ultima_factura_tip", "Ultima factura - tip", "mdi:file-document-outline", lambda d: _latest_billing_field(d, "invoiceType"), None, None, None, None),
        ("ultima_factura_scadenta", "Ultima factura - scadenta", "mdi:calendar-alert", lambda d: _latest_billing_field(d, "dueDate"), None, None, None, None),
        ("numar_factura", "Numar factura", "mdi:identifier", lambda d: _get_bill_result(d).get("invoicenumber") or None, None, None, None, None),
        ("consum_luna_curenta", "Consum luna curenta", "mdi:lightning-bolt", lambda d: _usage_value(_latest_usage_entry(d), "value"), UnitOfEnergy.KILO_WATT_HOUR, SensorDeviceClass.ENERGY, measurement, lambda d: _usage_month_label(_latest_usage_entry(d))),
        ("cost_consum_luna_curenta", "Cost consum luna curenta", "mdi:cash", lambda d: _usage_value(_latest_usage_entry(d), "UsageValue"), "RON", money, measurement, lambda d: _usage_month_label(_latest_usage_entry(d))),
        ("consum_mediu_zilnic", "Consum mediu zilnic", "mdi:chart-line", _current_month_daily_average, UnitOfEnergy.KILO_WATT_HOUR, SensorDeviceClass.ENERGY, measurement, lambda d: _usage_month_label(_latest_usage_entry(d))),
        ("total_consum_an_curent", "Total consum an curent", "mdi:sigma", lambda d: _current_year_usage_total(d, "value"), UnitOfEnergy.KILO_WATT_HOUR, SensorDeviceClass.ENERGY, total, None),
        ("total_cost_an_curent", "Total cost an curent", "mdi:cash-multiple", lambda d: _current_year_usage_total(d, "UsageValue"), "RON", money, total, None),
        ("numar_plati_an_curent", "Numar plati an curent", "mdi:counter", lambda d: _payment_count_current_year(d, "normal"), None, None, measurement, None),
        ("total_plati_an_curent", "Total plati an curent", "mdi:cash-check", lambda d: _payment_total_current_year(d, "normal"), "RON", money, total, None),
        ("pod", "POD", "mdi:map-marker-path", lambda d: _first_pod_field(d, "pod"), None, None, None, None),
        ("cod_incasare", "Cod incasare", "mdi:barcode", lambda d: _first_pod_field(d, "contractAccountID"), None, None, None, None),
        ("cod_partener_bp", "Cod partener BP", "mdi:account-key", lambda d: _first_pod_field(d, "accountID"), None, None, None, None),
        ("instalatie", "Instalatie", "mdi:transmission-tower", lambda d: _first_pod_field(d, "installation"), None, None, None, None),
        ("serie_contor", "Serie contor", "mdi:counter", _get_active_counter_series, None, None, None, None),
        ("operator_distributie", "Operator distributie", "mdi:domain", lambda d: _contract_field(d, "distributor"), None, None, None, None),
        ("furnizor", "Furnizor", "mdi:factory", lambda d: "Hidroelectrica" if _contract_field(d, "supplier") == "HE" else _contract_field(d, "supplier"), None, None, None, None),
        ("tip_contor", "Tip contor", "mdi:gauge", lambda d: _meter_field(d, "MeterType"), None, None, None, None),
        ("contor_inteligent", "Contor inteligent", "mdi:meter-electric", lambda d: "Da" if _meter_field(d, "IsAMI") == "True" else ("Nu" if _meter_field(d, "IsAMI") == "False" else _meter_field(d, "IsAMI")), None, None, None, None),
        ("fereastra_autocitire", "Fereastra autocitire", "mdi:calendar-check", _window_open, None, None, None, None),
        ("zi_deschidere_autocitire", "Zi deschidere autocitire", "mdi:calendar-start", lambda d: _get_window_data(d).get("OpeningDate") or None, None, None, None, None),
        ("zi_inchidere_autocitire", "Zi inchidere autocitire", "mdi:calendar-end", lambda d: _get_window_data(d).get("ClosingDate") or None, None, None, None, None),
        ("ultima_zi_transmitere_index", "Ultima zi transmitere index", "mdi:calendar-alert", _window_close_date, None, None, None, None),
        ("ultima_citire_index", "Ultima citire index", "mdi:counter", _latest_reading_field("Index", "1.8.0"), UnitOfEnergy.KILO_WATT_HOUR, SensorDeviceClass.ENERGY, measurement, None),
        ("ultima_citire_data", "Ultima citire data", "mdi:calendar", _latest_reading_field("Date", "1.8.0"), None, None, None, None),
        ("ultima_citire_tip", "Ultima citire tip", "mdi:clipboard-text-clock", _latest_reading_field("ReadingType", "1.8.0"), None, None, None, None),
        ("ultima_citire_registru", "Ultima citire registru", "mdi:format-list-bulleted-type", _latest_reading_field("Registers", "1.8.0"), None, None, None, None),
    ]

    if has_production:
        definitions.extend([
            ("total_compensatii_anre_an_curent", "Total compensatii ANRE an curent", "mdi:solar-power-variant", lambda d: _payment_total_current_year(d, "comp"), "RON", money, total, None),
            ("numar_compensatii_anre_an_curent", "Numar compensatii ANRE an curent", "mdi:counter", lambda d: _payment_count_current_year(d, "comp"), None, None, measurement, None),
            ("ultima_citire_productie_index", "Ultima citire productie index", "mdi:solar-power-variant", _latest_reading_field("Index", "1.8.0_P"), UnitOfEnergy.KILO_WATT_HOUR, SensorDeviceClass.ENERGY, measurement, None),
            ("ultima_citire_productie_data", "Ultima citire productie data", "mdi:calendar", _latest_reading_field("Date", "1.8.0_P"), None, None, None, None),
        ])

    return [
        HidroelectricaDetailSensor(
            coordinator,
            config_entry,
            key,
            name,
        ...
        for key, name, icon, value_fn, unit, device_class, state_class, attrs_fn in definitions
    ]
...
def _build_sensors_for_coordinator(
    ...
    sensors.extend(_build_additional_detail_sensors(coordinator, config_entry))
